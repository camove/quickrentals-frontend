...
const MyFlats = () => {
  ...
  const fetchMyFlatsWithURLParams = async (page, limit) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://quickrentals-backend.onrender.com/flats/my-flats?${params.toString()}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        setPagination({
          ...
          totalPages:
            data.totalPages || Math.ceil((flatsData.length || 0) / limit),
  ...
  const handleDeleteFlat = async () => {
    ...
    try {
      const response = await fetch(
        `https://quickrentals-backend.onrender.com/flats/${flatToDelete._id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    return (
      <div className={styles.container}>
        <div className={styles.loaderContainer}>
          ...
          <div className={styles.loadingText}>Loading your apartments...</div>
        </div>
      </div>
    );
  }
      ...
      {pagination.totalResults === 0 ? (
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyMessage}>
            Start by posting your first apartment to connect with potential
            tenants.
          </p>
      ...
      <ReusableModal
        ...
        message={
          <>
            ...
            <strong>
              {flatToDelete?.streetName} {flatToDelete?.streetNumber}
            </strong>
            ?
          </>
        }
